...
void non_preemptive(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    ...
    config(ready_queue, waiting_queue, compare, running, terminated_count, timer);
...
void FCFS(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    ...
    non_preemptive(ready_queue, waiting_queue, processes, process_count, running, terminated_count, timer, compare_ready_time);
}

void SJF_non_preemptive(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    int process_count, Process** running, int* terminated_count, int* timer){
    non_preemptive(ready_queue, waiting_queue, processes, process_count, running, terminated_count, timer, compare_cpu_remaining_time);
}

void SJF_preemptive(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    int process_count, Process** running, int* terminated_count, int* timer){
    preemptive(ready_queue, waiting_queue, processes, process_count, running, terminated_count, timer, compare_cpu_remaining_time);
}

void Priority_non_preemptive(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    int process_count, Process** running, int* terminated_count, int* timer){
    non_preemptive(ready_queue, waiting_queue, processes, process_count, running, terminated_count, timer, compare_priority);
}

void Priority_preemptive(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    int process_count, Process** running, int* terminated_count, int* timer){
    preemptive(ready_queue, waiting_queue, processes, process_count, running, terminated_count, timer, compare_priority);
}

void RR(Queue* ready_queue, Queue* waiting_queue, Process* processes, 
    int process_count, Process** running, int* terminated_count, int* timer){
    config(ready_queue, waiting_queue, compare_ready_time, running, terminated_count, timer);
    int q = TIME_QUANTUM;
